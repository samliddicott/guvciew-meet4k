...
capture_avi (GtkButton * CapAVIButt, GtkWidget * AVIFNameEntry)
{
	...
	if(videoIn->capAVI) {
	 ...
	 aviClose(); 		 
	   ...
	   /*4CC compression "YUY2" (YUV) or "RGB3" (RGB24) or "RGB4" (RGB32) or  whathever*/	
	   AVI_set_video(AviOut, videoIn->width, videoIn->height, videoIn->fps,"YUY2");		
...
int main_loop(void *data)
{
	...
	ImageSurf=SDL_CreateRGBSurface(SDL_SWSURFACE, overlay->w,
		   overlay->h, 32, 0x00ff0000,0x0000ff00,0x000000ff,0);
	...
	// SDL_WM_SetCaption(capt, NULL);
	 ...
	 if (videoIn->capImage){
		...
		Uint32 *pix = (Uint32 *) ImageSurf->pixels;
		pim= malloc((pscreen->w)*(pscreen->h)*sizeof(Uint32));
		...
		for (i=0;i<=(overlay->pitches[0])*(overlay->h)/4;i++){
			...
			pim[i*2]=SDL_MapRGB(ImageSurf->format, pix2->r,
		     pix2->g, pix2->b);
			pim[i*2+1]=SDL_MapRGB(ImageSurf->format, pix2->r1,
			 pix2->g1, pix2->b1); 
			
			//~ fwrite(&pix2->r, 1, 1, out);
			//~ fwrite(&pix2->g, 1, 1, out);
			//~ fwrite(&pix2->b, 1, 1, out);
			//~ fwrite(&pix2->r1, 1, 1, out);
			//~ fwrite(&pix2->g1, 1, 1, out);
			//~ fwrite(&pix2->b1, 1, 1, out);
			
		  }
	    
        SDL_LockSurface(ImageSurf);	
		memcpy(pix, pim,(pscreen->w)*(pscreen->h)*sizeof(Uint32));  
		SDL_UnlockSurface(ImageSurf);
		  
		//~ fclose(out);
	  
	    /* Now that we have a surface with the image we cam use SDL_Image */
	    /*  to convert to any of the formats available                    */
       	/* without SDL_Image only BMP is available                        */ 	  
	    if( SDL_SaveBMP(ImageSurf,videoIn->ImageFName) <0) { 
	  ...
	  long framesize= 	(pscreen->w)*(pscreen->h)*2; //YUY2 -> 2 bytes per pixel 
...
int main(int argc, char *argv[])
{
	...
	if (strcmp(argv[i], "-h") == 0) {
	    printf("usage: uvcview [-h -d -g -f -s -c -C -S] \n");
