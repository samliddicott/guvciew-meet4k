...
void MainWindow::menu_camera_button_clicked() {
  ...
  if (debug_level > 2)
    std::cout << "GUVCVIEW (Qt5): camera button set to action " << def_action
              << std::endl;
...
void MainWindow::load_save_profile_clicked() {
  ...
  if (dialog_type == 0) // load
  {
    ...
    if (!fileName.isEmpty()) {
      if (debug_level > 1)
        std::cout << "GUVCVIEW (Qt5): load profile " << fileName.toStdString()
                  << std::endl;

      v4l2core_load_control_profile(get_v4l2_device_handler(),
    ...
    if (!fileName.isEmpty()) {
      if (debug_level > 1)
        std::cout << "GUVCVIEW (Qt5): save profile " << fileName.toStdString()
                  << std::endl;

      v4l2core_save_control_profile(get_v4l2_device_handler(),
                                    fileName.toStdString().c_str());
...
void MainWindow::photo_sufix_clicked() {
  ...
  if (debug_level > 1)
    std::cout << "GUVCVIEW (Qt5): photo sufix set to " << flag << std::endl;
...
void MainWindow::video_sufix_clicked() {
  ...
  if (debug_level > 1)
    std::cout << "GUVCVIEW (Qt5): video sufix set to " << flag << std::endl;
...
void MainWindow::video_codec_clicked() {
  ...
  if (debug_level > 1)
    std::cout << "GUVCVIEW (Qt5): video codec changed to " << vcodec_ind
              << std::endl;
...
void MainWindow::audio_codec_clicked() {
  ...
  if (debug_level > 1)
    std::cout << "GUVCVIEW (Qt5): audio codec changed to " << acodec_ind
              << std::endl;
...
void MainWindow::photo_file_clicked() {
  ...
  QString fileName = QFileDialog::getSaveFileName(
      this, _("Photo file name"), photo_name, filter, &filter_all);
...
void MainWindow::video_file_clicked() {
  ...
  QString fileName = QFileDialog::getSaveFileName(
      this, _("Video file name"), video_name, filter, &filter_all);
...
void MainWindow::capture_video_clicked() {
  ...
  if (debug_level > 0)
    std::cout << "GUVCVIEW (Qt5): video capture cliked: " << is_active
              << std::endl;
...
void MainWindow::button_PanTilt1_clicked() {
  ...
  v4l2_ctrl_t *control =
      v4l2core_get_control_by_id(get_v4l2_device_handler(), id);
...
void MainWindow::string_button_clicked() {
  ...
  int id = sender->property("control_info").toInt();
  QLineEdit *entry =
      (QLineEdit *)sender->property("control_entry").value<QLineEdit *>();
  ...
  strncpy(control->string, text_input.toStdString().c_str(),
          control->control.maximum);

  if (v4l2core_set_control_value_by_id(get_v4l2_device_handler(), id))
    std::cerr << "GUVCVIEW (Qt5): error setting string value" << std::endl;
}
...
void MainWindow::int64_button_clicked() {
  ...
  if (!entry) {
    std::cerr
        << "Guvcview (Qt5): couldn't get QLineEdit pointer for int64 control: "
...
void MainWindow::bitmask_button_clicked() {
  ...
  if (ok) {
    control->value = value;
    if (v4l2core_set_control_value_by_id(get_v4l2_device_handler(), id))
      std::cerr << "GUVCVIEW (Qt5): error setting bitmask value" << std::endl;
  } else
...
void MainWindow::devices_changed(int index) {
  ...
  QPushButton *restartButton =
      msgBox.addButton(_("restart"), QMessageBox::ActionRole);
  ...
  combobox_video_devices->setCurrentIndex(
...
void MainWindow::frame_rate_changed(int index) {
  ...
  int resolu_index = v4l2core_get_format_resolution_index(
      get_v4l2_device_handler(), format_index,
      v4l2core_get_frame_width(get_v4l2_device_handler()),
      v4l2core_get_frame_height(get_v4l2_device_handler()));

  v4l2_stream_formats_t *list_stream_formats =
      v4l2core_get_formats_list(get_v4l2_device_handler());

  int fps_denom = list_stream_formats[format_index]
                      .list_stream_cap[resolu_index]
                      .framerate_denom[index];
  int fps_num = list_stream_formats[format_index]
                    .list_stream_cap[resolu_index]
                    .framerate_num[index];
...
void MainWindow::resolution_changed(int index) {
  ...
  for (i = 0;
       i < list_stream_formats[format_index].list_stream_cap[index].numb_frates;
       i++) {
    QString fps_str = QString("%1/%2 fps")
                          .arg(list_stream_formats[format_index]
                                   .list_stream_cap[index]
                                   .framerate_denom[i])
                          .arg(list_stream_formats[format_index]
                                   .list_stream_cap[index]
                                   .framerate_num[i]);
    ...
    if ((v4l2core_get_fps_num(get_v4l2_device_handler()) ==
         list_stream_formats[format_index]
             .list_stream_cap[index]
             .framerate_num[i]) &&
        (v4l2core_get_fps_denom(get_v4l2_device_handler()) ==
         list_stream_formats[format_index]
             .list_stream_cap[index]
             .framerate_denom[i]))
  ...
  if (list_stream_formats[format_index].list_stream_cap[index].framerate_num)
    v4l2core_define_fps(get_v4l2_device_handler(),
                        list_stream_formats[format_index]
                            .list_stream_cap[index]
                            .framerate_num[deffps],
                        -1);

  if (list_stream_formats[format_index].list_stream_cap[index].framerate_denom)
    v4l2core_define_fps(get_v4l2_device_handler(), -1,
                        list_stream_formats[format_index]
                            .list_stream_cap[index]
                            .framerate_denom[deffps]);
...
void MainWindow::format_changed(int index) {
  ...
  for (i = 0; i < list_stream_formats[index].numb_res; i++) {
    if (list_stream_formats[index].list_stream_cap[i].width > 0) {
      QString res_str =
          QString("%1x%2")
              .arg(list_stream_formats[index].list_stream_cap[i].width)
              .arg(list_stream_formats[index].list_stream_cap[i].height);
      combobox_resolution->addItem(res_str, i);

      if ((v4l2core_get_frame_width(get_v4l2_device_handler()) ==
...
void MainWindow::render_fx_filter_changed(int state) {
  ...
  uint32_t mask = (state != 0) ? get_render_fx_mask() | filter
...
void MainWindow::audio_fx_filter_changed(int state) {
  ...
  uint32_t mask = (state != 0) ? get_audio_fx_mask() | filter
...
void MainWindow::render_osd_changed(int state) {
  ...
  uint32_t mask =
      (state != 0) ? render_get_osd_mask() | osd : render_get_osd_mask() & ~osd;
...
void MainWindow::autofocus_changed(int state) {
  ...
  // GtkWidget *wgtFocus_slider = (GtkWidget *) g_object_get_data (G_OBJECT
  // (toggle), "control_entry"); GtkWidget *wgtFocus_spin = (GtkWidget *)
  // g_object_get_data (G_OBJECT (toggle), "control2_entry");
 ...
 * returns: none
 */
void MainWindow::setfocus_clicked() { set_soft_focus(1); }
...
void MainWindow::audio_devices_changed(int index) {
  ...
  if (chan_ind == 0) {
    int channels =
        audio_get_device(audio_ctx, audio_get_device_index(audio_ctx))
            ->channels;
    if (channels > 2)
...
void MainWindow::audio_samplerate_changed(int index) {
  ...
  switch (index) {
  case 0:
    samprate = audio_get_device(audio_ctx, audio_get_device_index(audio_ctx))
                   ->samprate;
    break;
  default:
...
void MainWindow::audio_channels_changed(int index) {
  ...
  switch (index) {
  case 0:
    channels = audio_get_device(audio_ctx, audio_get_device_index(audio_ctx))
                   ->channels;
    break;
  default:
  ...
  if (channels >
      audio_get_device(audio_ctx, audio_get_device_index(audio_ctx))->channels)
    channels = audio_get_device(audio_ctx, audio_get_device_index(audio_ctx))
                   ->channels;
...
void MainWindow::audio_api_changed(int index) {
  ...
  if (index == AUDIO_NONE || audio_ctx == NULL) {
    ...
    for (i = 0; i < audio_get_num_inp_devices(audio_ctx); ++i) {
      combobox_audio_devices->addItem(
          audio_get_device(audio_ctx, i)->description, i);
    }
    ...
    std::cout << "GUVCVIEW (Qt5): setting default audio latency to "
              << audio_get_latency(audio_ctx) << std::endl;
...
void MainWindow::video_codec_properties() {
  video_codec_t *defaults =
      encoder_get_video_codec_defaults(get_video_codec_ind());
...
void MainWindow::audio_codec_properties() {
  audio_codec_t *defaults =
      encoder_get_audio_codec_defaults(get_audio_codec_ind());
...
void MainWindow::keyPressEvent(QKeyEvent *e) {
  ...
  std::cout << "GUVCIVEW (Qt5): " << message.toStdString() << "(" << e->key()
            << ")" << std::endl;
