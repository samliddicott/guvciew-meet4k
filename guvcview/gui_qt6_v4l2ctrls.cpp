 ...
 */
ControlWidgets *MainWindow::gui_qt6_get_widgets_by_id(int id) {
  unsigned int i = 0;
 ...
 */
int MainWindow::gui_attach_qt6_v4l2ctrls(QWidget *parent) {
  /*assertions*/
  assert(parent != NULL);
  ...
  for (; current != NULL; current = current->next, ++n) {
    if (current == NULL) {
      std::cerr << "GUVCVIEW: ERROR (attach qt5 controls) empty control in list"
                << std::endl;
      break;
    }
    ...
    case V4L2_CTRL_TYPE_INTEGER:
      ...
      case V4L2_CID_TILT_RELATIVE: {
        ...
        /*connect signals*/
        connect(PanTilt1, SIGNAL(released()), this,
                SLOT(button_PanTilt1_clicked()));
        connect(PanTilt2, SIGNAL(released()), this,
                SLOT(button_PanTilt2_clicked()));
        ...
        connect(spinbox, SIGNAL(valueChanged(int)), this,
                SLOT(pan_tilt_step_changed(int)));
      ...
      case V4L2_CID_LED1_MODE_LOGITECH: {
        ...
        if (!current->menu)
          current->menu =
              (v4l2_querymenu *)calloc(4 + 1, sizeof(struct v4l2_querymenu));
        else
          current->menu = (v4l2_querymenu *)realloc(
              current->menu, (4 + 1) * sizeof(struct v4l2_querymenu));

        if (current->menu == NULL) {
          std::cerr << "GUVCVIEW: FATAL memory allocation failure "
                       "(gui_attach_qt_v4l2ctrls):"
        ...
        for (j = 0; (int)current->menu[j].index <= current->control.maximum;
             j++) {
      ...
      case V4L2_CID_RAW_BITS_PER_PIXEL_LOGITECH: {
        ...
        if (!current->menu)
          current->menu =
              (v4l2_querymenu *)calloc(2 + 1, sizeof(struct v4l2_querymenu));
        else
          current->menu = (v4l2_querymenu *)realloc(
              current->menu, (2 + 1) * sizeof(struct v4l2_querymenu));
        if (current->menu == NULL) {
          fprintf(stderr,
                  "GUVCVIEW: FATAL memory allocation failure "
                  "(gui_attach_gtk3_v4l2ctrls): %s\n",
                  strerror(errno));
          exit(-1);
        }
        current->menu[0].id = current->control.id;
      ...
      case V4L2_CID_FOCUS_ABSOLUTE:
        ...
        if (!is_control_panel) {
          ...
          QCheckBox *checkbox =
              new QCheckBox(_("Auto Focus (continuous)"), img_controls_grid);
          ...
          connect(checkbox, SIGNAL(stateChanged(int)), this,
                  SLOT(autofocus_changed(int)));
          connect(pushbutton, SIGNAL(released()), this,
                  SLOT(setfocus_clicked()));
      ...
      default: /*standard case - slider + spin*/
      {
        ...
        if ((current->control.maximum > current->control.minimum) &&
            (current->control.step != 0)) {
          ...
          // connect(spinbox,SIGNAL(valueChanged(int)),this,
          // SLOT(spin_value_changed(int)));

          // if(!is_control_panel &&
          //    (current->control.id == V4L2_CID_FOCUS_LOGITECH ||
          //     current->control.id == V4L2_CID_FOCUS_ABSOLUTE))
          //{
          //	g_object_set_data (G_OBJECT (control_widgets_list[n-1].widget),
          //"control_entry", 		control_widgets_list[n].widget); 	g_object_set_data
          //(G_OBJECT (control_widgets_list[n-1].widget), "control2_entry",
          //		control_widgets_list[n].widget2);
          // }

        } else
          std::cerr
              << "GUVCVIEW (Qt5): (Invalid range) [MAX <= MIN] for control id:"
      ...
      if (current->menu) {
        ...
        for (j = 0; (int)current->menu[j].index <= current->control.maximum;
          ...
          if (current->control.type == V4L2_CTRL_TYPE_MENU) {
            combobox->addItem((char *)current->menu_entry[j],
                              current->menu[j].index);
          } else {
            QString text_input =
                QString("%1").arg(current->menu[j].value, 4, 10, QChar('0'));
            combobox->addItem(text_input,
                              QVariant::fromValue(current->menu[j].value));
          }
          if (current->value == (int)current->menu[j].index)
            def = j;
        }
    ...
    case V4L2_CTRL_TYPE_BOOLEAN: {
      if (current->control.id == V4L2_CID_DISABLE_PROCESSING_LOGITECH) {
        ...
        combobox->setCurrentIndex(
            v4l2core_get_bayer_pix_order(get_v4l2_device_handler()));
        thisone->widget2->show();

        /*connect signal*/
        connect(combobox, SIGNAL(currentIndexChanged(int)), this,
                SLOT(bayer_pix_ord_changed(int)));
      ...
      /*connect signal*/
      connect(checkbox, SIGNAL(stateChanged(int)), this,
              SLOT(check_changed(int)));
    } break;

    default:
      printf("control[%d]:(unknown - 0x%x) 0x%x '%s'\n", i,
             current->control.type, current->control.id, current->control.name);
      break;
    }
 ...
 * returns: none
 */
void MainWindow::gui_qt6_update_controls_state() {
  v4l2_ctrl_t *current = v4l2core_get_control_list(get_v4l2_device_handler());
  ...
  for (; current != NULL; current = current->next) {
    if (current == NULL) {
      fprintf(
          stderr,
          "GUVCVIEW: ERROR (update controls state) empty control in list\n");
      break;
    }

    ControlWidgets *thisone = gui_qt6_get_widgets_by_id(current->control.id);

    if (!thisone) {
      std::cerr << "GUVCVIEW (Qt5): (update widget state): control " << std::hex
                << current->control.id << std::dec
    ...
    case V4L2_CTRL_TYPE_STRING: {
      if (current->string != NULL) {
        ...
        std::cerr << "GUVCVIEW (Qt5): control " << std::hex
                  << current->control.id << std::dec
    ...
    case V4L2_CTRL_TYPE_BITMASK: {
      ...
      text_input.append(
          QString::number((uint32_t)current->value, 16).toUpper());
      QLineEdit *entry = (QLineEdit *)thisone->widget;
      entry->setText(text_input);
    } break;
