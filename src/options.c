...
writeConf(struct GLOBAL *global, char *videodevice) 
{
	...
	if ((fp = g_fopen(global->confPath,"w"))!=NULL) 
	{
		...
		g_fprintf(fp,"# mode video format 'yuvy' 'yvyu' 'uyvy' 'yyuv' 'yu12' 'yv12' 'nv12' 'nv21' 'nv16' 'nv61' 'y41p' 'grey' 'gbrg' 'grbg' 'bggr' 'rggb' 'rgb3' 'bgr3' 'jpeg' 'mjpg' 'mjpg'(default)\n");
		g_fprintf(fp,"mode='%s'\n",global->mode);
...
readOpts(int argc,char *argv[], struct GLOBAL *global)
{
	...
	GOptionEntry entries[] =
	{
		...
		{ "format", 'f', 0, G_OPTION_ARG_STRING, &format, N_("Pixel format(mjpg|jpeg|yuyv|yvyu|uyvy|yyuv|yu12|yv12|nv12|nv21|nv16|nv61|y41p|grey|gbrg|grbg|ba81|rggb|bgr3|rgb3)"), "FORMAT" },
